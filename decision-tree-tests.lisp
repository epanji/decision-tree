...
(cl:in-package #:decision-tree/tests)

(defmacro with-new-decision-trees (&body body)
  `(let ((*decision-trees* (make-hash-table :test 'equal)))
     ,@body))

(defun unordered-equal (list1 list2)
  (when (and (typep list1 '(cons atom))
             (typep list1 '(cons atom)))
    (and (= (length list1) (length list2))
         (= (length list1) (length (union list1 list2 :test 'equal))))))
...
(test answer-response ()
  (with-dummy-data ()
    ...
    (is (every (lambda (it)
                 (find it '(("d-1" "c-3" "c-1") ("d-2" "c-3") ("d-3"))
                       :test 'unordered-equal))
               (relations decision-tree)))
...
;; DONE swap left and right
;; Need to compile and run from clim listener
#+(or)
(defun visual-tree (&optional swap-p stream)
  (with-dummy-data ()
    (decision-to-tree "test" d1)
    (decision-to-tree "test" d2)
    (decision-to-tree "test" d3)
    ...
    (criteria-to-decision-in-tree "test" "d-3" "c-2")
    (criteria-to-decision-in-tree "test" "d-4" "c-3")
    (let ((tree (let ((*code-tree-order* (if swap-p :c-y-n :c-n-y)))
                  (code-tree decision-tree))))
       ...
       (lambda (obj stream)
         (let ((obj (typecase obj (list (first obj)) (t obj))))
           (let ((fm (clim:frame-manager clim:*application-frame*)))
             (clim:with-look-and-feel-realization (fm clim:*application-frame*)
               (clim:with-output-as-gadget (stream)
                 (clim:make-pane 'clim:push-button
                                 :activate-callback
                                 (lambda (&rest args)
                                   (declare (ignore args))
                                   (clim:notify-user clim:*application-frame* "You clicked a button."))
